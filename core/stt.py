...
def subtitle_generation(speech_to_text_response, bin_size=3):
    ...
    print("words: ", len(words_real))
    print("timest: " ,len(timestamps_real))
    start_index = 0
    final_index = 0
    sen = ""
    for i in range(len(words_real)):
        ...
        if(words[i][-1] in ['.', '?', '!', '다', '요','까', '죠']  or i == len(words) -1):
            final_index = i
            index += 1
            transcriptions.append(srt.Subtitle(index, datetime.timedelta(0, timestamps_real[start_index][0], timestamps_real[start_index][1]), datetime.timedelta(0, timestamps_real[final_index][2], timestamps_real[final_index][3]), sen))
            sen = ""
            start_index = i+1
